...
describe("JoinWaitList Component", () => {
  ...
  test("shows error if user is not logged in", () => {
    //clear localStorage to simulate user not logged in
    window.localStorage.clear();
  ...
  test("does not allow joining waitlist if user is not logged in", () => {
    window.localStorage.clear(); 
    ...
    expect(screen.getByText(/Please log in to join the waitlist./i)).toBeInTheDocument();
  });
});
